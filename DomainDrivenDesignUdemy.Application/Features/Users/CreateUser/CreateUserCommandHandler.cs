...
internal sealed class CreateUserCommandHandler : IRequestHandler<CreateUserCommand>
{
    ...
    {
        // business logic
        var user = await _userRepository.CreateAsync(request.Name, request.Email, request.Password, request.Country, request.City,
            request.Street, request.PostalCode, request.FullAddress, cancellationToken);
